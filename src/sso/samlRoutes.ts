		...
		new Elysia()
			...
			.get(
				...
				async ({ params: { organizationId }, request, status }) => {
					...
					const metadata =
						await samlAdapter.getServiceProviderMetadata({
							acsUrl: acsUrlFor(request.url, organizationId),
							connection,
			...
			.get(
				...
				async ({
					...
					const buildLogoutRequest =
						samlAdapter.createLogoutRequestUrl;
					if (
						connection?.type === 'saml' &&
						idpSloUrl !== undefined &&
			...
			.get(
				...
				async ({
					...
					const signedQueryString = new URL(
						request.url
					).search.replace(/^\?/, '');
					...
					const buildLogoutResponse =
						samlAdapter.createLogoutResponseUrl;
					...
					return redirect(
						toSafeLocalPath(info.relayState ?? RelayState)
					);
				}
			)
	);
